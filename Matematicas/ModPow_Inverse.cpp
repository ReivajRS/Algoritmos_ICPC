int mod(int a, int m) {                          // returns a (mod m)
  return ((a%m) + m) % m;                        // Asegura una respuesta positiva
}

int modPow(int b, int p, int m) {                // Asume que 0 <= b < m
  if (p == 0) return 1;
  int ans = modPow(b, p/2, m);                   // Esto es O(log p)
  ans = mod(ans*ans, m);                         // Primero se duplica
  if (p&1) ans = mod(ans*b, m);                  // *b si p es impar
  return ans;                                    // ans siempre en [0..m-1]
}
